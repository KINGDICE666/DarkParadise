...
/datum/controller/subsystem/persistent_paintings/proc/update_format(current_data)
	...
	if(version < 3) //Reduces the allowed length of titles from 1000 characters circa to 42.
		...
		var/new_title = old_title// reject_bad_name(old_title, allow_numbers = TRUE, ascii_only = FALSE, strict = TRUE, cap_after_symbols = FALSE)
...
/datum/controller/subsystem/persistent_paintings/proc/migrate_to_version_1(list/current_data)
	...
	for(var/category in current_data)
		for(var/old_data in current_data[category])
			...
			new_data["creation_date"] = time2text(world.realtime, "DDD MMM DD hh:mm:ss YYYY") // Could use creation/modified file helpers in rustg
