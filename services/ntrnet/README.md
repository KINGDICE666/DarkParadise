# НТрнет: API, вход и редактор сайтов
...
Каталог и страницы из базы совместимы с PDA. В браузере доступны вход, создание
сайтов и страниц, редактор HTML/CSS, безопасный предпросмотр и загрузка медиа на
локальный диск VDS. Авторский HTML никогда не передаётся в игру: сервис преобразует
его в ограниченное JSON-дерево, а TGUI повторно проверяет типы, стили и адреса медиа.
...
../../.cache/ntrnet-venv/Scripts/python manage.py migrate
...
## Редактор и ограничения

- Игрок может создать до трёх сайтов и до двадцати страниц в каждом. Адрес сайта
  содержит 3–24 символа; смешивать латиницу и кириллицу нельзя. Зона выбирается
  из общей `.ss13` и зоны сервера, который подтвердил вход.
- Поддерживаются `div`, `p`, `span`, заголовки, простое оформление текста, списки,
  цитаты, код, разделители и внутренние ссылки. CSS принимает только простые
  селекторы и белый список свойств. `script`, `iframe`, `on*`, атрибут `style`,
  внешние ссылки, `url()`, `@import` и неизвестная разметка отклоняются целиком.
- Готовое дерево ограничено 64 КБ, 1024 элементами и глубиной 16. Ошибка проверки
  не заменяет опубликованную страницу. Версия сайта увеличивается только после
  успешного сохранения.
- Предпросмотр доступен по длинной секретной ссылке автора, не содержит каталога
  и закрыт от индексации. Ссылку можно отозвать одной кнопкой.
- Сайт удаляется только после ввода его полного адреса; вместе с ним удаляются
  страницы и принадлежащие ему медиафайлы.
- Медиа: 50 МБ и 100 файлов на сайт; PNG/JPEG/GIF/WebP до 10 МБ и 4096×4096,
  MP4 до 30 МБ. Изображения полностью проверяются через Pillow, а MP4
  через `ffprobe`: одна видеодорожка H.264, звуковые дорожки только AAC. SVG и
  внешние источники не принимаются. Используемый на странице файл удалить нельзя.
...
3. Создайте `/etc/ntrnet.env` по `deploy/ntrnet.env.example`, права root:root 0600.
   ...
   `manage.py init-db`, `manage.py migrate` и
   `manage.py register-server dark-paradise 'Dark Paradise' ss13dp`.
   После инициализации используйте ограниченную учётную запись БД.
4. Установите `python3`, `python3-venv`, `ffmpeg`, `unzip` и nginx. Чтобы получить
   один архив для загрузки, из корня репозитория выполните
   `powershell -ExecutionPolicy Bypass -File services/ntrnet/deploy/build-release.ps1`.
   Загрузите `.cache/ntrnet-release.zip`, распакуйте его содержимое в `/srv/ntrnet`
   и выполните `bash /srv/ntrnet/deploy/install.sh`. Скрипт устанавливает точные
   зависимости, применяет схему, ставит три systemd unit-файла и запускает сервис.
...
5. Настройте TLS и nginx по разделу ниже. Порт 8091 и MariaDB не открывайте наружу.
   ...
   Настройте резервное копирование базы и `/srv/ntrnet-media` вместе с существующими
   резервными копиями VDS.
...
Для медиа создайте проксируемую A-запись `media.wiki-ss13.space` на адрес VDS.
Приложение сохраняет проверенные файлы в `/srv/ntrnet-media`, а отдельный nginx
virtual host раздаёт их только для чтения. `install.sh` создаёт каталог с владельцем
`ntrnet`; systemd разрешает сервису запись только в этот путь. В сертификате должны
быть оба имени: `ntrnet.wiki-ss13.space` и `media.wiki-ss13.space` (либо wildcard).
Имена файлов неизменяемые и случайные, поэтому Cloudflare может кэшировать их год.
Каталог медиа обязательно включите в резервные копии и контроль свободного места VDS.
...
Проверено 17.09.2026: 34/34 теста API, редактора, санитайзера и миграции прошли
на MariaDB 11.4.8; на SQLite прошли 33, а тест production-cookie пропущен намеренно.
В браузере проверены вход, создание сайта, редактирование и предпросмотр.
...
TGUI: сборка production, TypeScript, ESLint и восемь тестов рендерера прошли;
