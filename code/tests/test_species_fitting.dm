...
/datum/unit_test/species_fitting_sheet_maps/Run()
	...
	var/list/boot_pixels = fit.read_frame(icon(fit.fit_worn_icon(boots, DEFAULT_ICON_SHOES, "jackboots"), dir = SOUTH))
	var/list/feet = fit.build_mask(fit.target_sheet, list("l_foot", "r_foot"), SOUTH)
	for(var/index in 1 to length(feet))
		if(feet[index])
			TEST_ASSERT_NOTNULL(boot_pixels[index], "jackboots leave resomi toes bare at pixel [index]")
