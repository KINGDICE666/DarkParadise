...
/datum/element/cliff_walking/Attach(datum/target)
	. = ..()
	if(!isliving(target))
		return ELEMENT_INCOMPATIBLE

	RegisterSignal(target, COMSIG_LIVING_DEATH, PROC_REF(update_cliff_walking))
	RegisterSignal(target, COMSIG_LIVING_REVIVE, PROC_REF(update_cliff_walking))
...
/datum/element/cliff_walking/Detach(datum/source, ...)
	...
	REMOVE_TRAIT(source, TRAIT_CLIFF_WALKER, type)
	return ..()
...
/datum/element/cliff_walking/proc/update_cliff_walking(mob/living/climber)
	...
	var/turf/simulated/floor/cliff/cliff_tile = get_turf(climber)
