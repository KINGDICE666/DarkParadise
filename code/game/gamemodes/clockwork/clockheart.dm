...
/obj/structure/clockwork/functional/heart/Initialize(mapload)
	...
	spawn_parts()
	get_clockwork_cult_team().clocker_objs.check_heart()
	. = ..()
...
/obj/structure/clockwork/functional/heart/Destroy(force)
	...
	if(clock_team.clocker_objs.clock_status != RATVAR_NEEDS_SUMMONING && clock_team.clocker_objs.clock_status != RATVAR_HAS_RISEN)
...
/obj/structure/clockwork/functional/heart/mouse_drop_receive(atom/movable/dropping, mob/user, params)
	...
	get_clockwork_cult_team().check_clock_reveal()
	qdel(dropping)
	update_icon(UPDATE_OVERLAYS)
...
/obj/structure/clockwork/functional/heart/proc/adjust_part(obj/item/part_upper/part, mob/user)
	...
	qdel(part)
	update_icon(UPDATE_OVERLAYS)
	get_clockwork_cult_team().clocker_objs.update_seals()

/obj/structure/clockwork/functional/heart/proc/summon(mob/user, obj/item/shard)
	if(GLOB.total_curses > 0)
		balloon_alert(user, "сначала снимите печати")
		return
	if(get_clockwork_cult_team().clocker_objs.clock_status < RATVAR_NEEDS_SUMMONING)
