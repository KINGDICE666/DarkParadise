...
/proc/get_all_accesses()
	return list(ACCESS_MINISAT, ACCESS_AI_UPLOAD,  ACCESS_ARMORY, ACCESS_ATMOSPHERICS, ACCESS_BAR, ACCESS_SEC_DOORS, ACCESS_BLUESHIELD,
				...
				ACCESS_XENOARCH, ACCESS_INVESTOR, ACCESS_BITRUNNING)
...
/proc/get_region_accesses(code)
	switch(code)
		...
		if(REGION_SUPPLY) //supply
			return list(ACCESS_MAILSORTING, ACCESS_MINING, ACCESS_MINING_STATION, ACCESS_MINERAL_STOREROOM, ACCESS_CARGO, ACCESS_BITRUNNING, ACCESS_QM)
...
/proc/get_access_desc(A)
	switch(A)
		...
		if(ACCESS_MINING)
			return "Mining"
		if(ACCESS_BITRUNNING)
			return "Bitrunning Den"
