...
/datum/action/cooldown/spell/aoe/rust_conversion/cast(atom/cast_on)
	. = ..()
	for(var/atom/victim as anything in get_things_to_cast_on(owner))
		var/distance_to_caster = get_dist(victim, owner)
		...
		if(ismob(owner))
			owner.do_rust_heretic_act(victim)
		else
