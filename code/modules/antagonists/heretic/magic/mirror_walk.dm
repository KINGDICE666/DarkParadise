...
/obj/effect/proc_holder/spell/jaunt/mirror_walk/can_cast(mob/user = usr, charge_check = TRUE, show_message = TRUE)
	. = ..()
	if(!.)
		return FALSE
...
/obj/effect/proc_holder/spell/jaunt/mirror_walk/on_jaunt_exited(obj/effect/dummy/spell_jaunt/jaunt, mob/living/unjaunter)
	...
	if(is_space_or_openspace(phase_turf))
		var/datum/gas_mixture/phase_air = phase_turf.return_air()
		phase_air.set_temperature(max(0, phase_air.temperature() - 20))
