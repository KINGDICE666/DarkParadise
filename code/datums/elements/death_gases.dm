...
/datum/element/death_gases/proc/on_death(mob/living/target, gibbed)
	...
	switch(gas_type)
		if(LINDA_SPAWN_TOXINS)
			mix_to_spawn.set_toxins(mix_to_spawn.toxins() + amount_of_gas)

		if(LINDA_SPAWN_OXYGEN)
			mix_to_spawn.set_oxygen(mix_to_spawn.oxygen() + amount_of_gas)

		if(LINDA_SPAWN_CO2)
			mix_to_spawn.set_carbon_dioxide(mix_to_spawn.carbon_dioxide() + amount_of_gas)

		if(LINDA_SPAWN_NITROGEN)
			mix_to_spawn.set_nitrogen(mix_to_spawn.nitrogen() + amount_of_gas)

		if(LINDA_SPAWN_N2O)
			mix_to_spawn.set_sleeping_agent(mix_to_spawn.sleeping_agent() + amount_of_gas)

		if(LINDA_SPAWN_AGENT_B)
			mix_to_spawn.set_agent_b(mix_to_spawn.agent_b() + amount_of_gas)
