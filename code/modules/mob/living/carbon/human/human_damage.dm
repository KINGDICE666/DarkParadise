...
/mob/living/carbon/human/adjustBrainLoss(
	...
	if(dna.species.has_organ[INTERNAL_ORGAN_BRAIN])
		...
		if(sponge)
			var/previous_damage = sponge.damage
			sponge.damage = clamp(round(sponge.damage + amount, DAMAGE_PRECISION), 0, sponge.max_damage)
			sponge.on_damage_changed(previous_damage)
			if(sponge.damage >= sponge.max_damage && stat != DEAD)
...
/mob/living/carbon/human/setBrainLoss(amount, updating_health = TRUE)
	...
	if(dna.species.has_organ[INTERNAL_ORGAN_BRAIN])
		...
		if(sponge)
			var/previous_damage = sponge.damage
			sponge.damage = clamp(round(amount, DAMAGE_PRECISION), 0, sponge.max_damage)
			sponge.on_damage_changed(previous_damage)
			if(sponge.damage >= sponge.max_damage && stat != DEAD)
				visible_message(span_alert("<b>[DECLENT_RU_CAP(src, NOMINATIVE)]</b> обмяк[GEND_LA_LO_LI(src)], [GEND_HIS_HER(src)] выражение лица совершенно пустое."))
				death()
