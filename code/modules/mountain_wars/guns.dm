...
// delete_casings стоит на всём скорострельном оружии режима. Флаг объявлен на
// /obj/item/gun/projectile и по умолчанию выключен — станционные стволы ведут себя как
// вели. Гильза с ним удаляется, а не падает на пол: каждая упавшая — живой объект с
